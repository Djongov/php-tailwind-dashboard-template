...
class DataGrid
{
    public static function createTable(string $dbTable, array $data, string $theme, bool $edit = true, bool $delete = true): string
    {
        ...
        $html .= '
        <div id="' . $title . '-delete-loading-screen" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-72 h-28 bg-slate-50 dark:bg-slate-600 hidden border border-black dark:border-slate-200">
            <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-row">
            <p id="' . $title . '-delete-loading-screen-text" class="text-' . $theme . '-500">Deleting...</p>
                <div role="status">
        ...
        if ($totalCount < 1) {
            $html .= HTML::h3('No results for "' . $originalTitle . '"', true);
            return $html;
        }
        // All encompassing div
        $html .= '<div class="my-4">';
        $html .= '<div class="ml-2 mt-4 text-gray-700 dark:text-gray-400">';
        $html .= '<h1 class="text-2xl text-center font-bold text-black dark:text-gray-400">' . $originalTitle . '</h1>';
        $html .= '<p>Results: <span id="' . $title . '-total">' . $totalCount . '</span></p>';
        $html .= '<p>Filtered: <span id="' . $title . '-filtered">' . $totalCount . '</span></p>';
        $html .= '<p>Selected: <span id="' . $title . '-selected">0</span></p>';
        $html .= '</div>';
        // Table
        $html .= '<div id="' . $title . '-container" class="m-4">';
        $html .= '<form class="delete-selected-form" method="post" action="">';
        // The table loading div
        $html .= '<div id="' . $title . '-loading-table" class="mt-12 bg-' . $theme . '-500 h-full w-full text-center text-white">Data Loading... Please wait<svg class="inline mx-4 w-8 h-8 text-gray-200 dark:text-white animate-spin fill-blue-600 dark:fill-' . $theme . '-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/></svg></div>';
        // Table itself
        $html .= '<table id="' . $title . '" class="hidden w-full bg-gray-100 dark:bg-gray-900 datagrid mt-8 p-8 text-gray-700 dark:text-gray-400 border-collapse border border-slate-400 text-center">';
        ...
        if ($delete) {
            ...
            $html .= '<button id="' . $title . '-mass-delete-modal-trigger" class="delete-selected block ml-2 my-2 text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" type="button" data-modal-toggle="' . $title . '-mass-delete-modal">
                Delete selected
            </button>';
            $html .= '
            <div id="' . $title . '-mass-delete-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0 h-modal md:h-full justify-center items-center" aria-hidden="true">
                <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                    <div class="relative bg-white border border-gray-400 dark:border-gray-300 rounded-lg shadow dark:bg-gray-700">
                        <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="' . $title . '-mass-delete-modal">
                        ...
                        <div class="p-6 text-center">
                            ...
                            <h3 id="' . $title . '-mass-delete-modal-text" class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this entry?</h3>
                            <button data-modal-toggle="' . $title . '-mass-delete-modal" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                Yes, I\'m sure
                            </button>
                            <button data-modal-toggle="' . $title . '-mass-delete-modal" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            $html .= '<input type="hidden" name="deleteRecords" value="' . $originalTitle . '" />';
        ...
        $html .= <<<EOL
            ...
            $(document).ready(() => {
                const table = drawDataGrid('$title');
                buildDataGridFilters(table, '$title', $skipColumnArray);
