...
class Init
{
    private function head(string $title, array $keywords = GENERIC_KEYWORDS, string $description = GENERIC_DESCRIPTION, string $thumbimage = OG_LOGO): string
    {
        ...
        //$html .= '<script src="/assets/js/main.js?'. time() . '"></script>' . PHP_EOL;
    ...
    {
        // if the menu array is empty, return nothing
        if (empty($array)) {
            return;
        }
        ...
        if (isset($usernameArray['username']) && $usernameArray['username'] !== null || str_contains($_SERVER['REQUEST_URI'], '/demo')) {
            ...
            foreach (USERNAME_DROPDOWN_MENU as $name => $array) {
                // if in /demo
                if (str_contains($_SERVER['REQUEST_URI'], '/demo')) {
                    // prepend /demo to the path
                    $array['path'] = '/demo' . $array['path'];
                }
    ...
    private function insertController($filePath, $usernameArray, $username, $loggedIn, $isAdmin, $theme)
    {
        ...
        // Include the file
        include_once dirname($_SERVER['DOCUMENT_ROOT']) . '/resources/views/' . $filePath;
    ...
    private function footer($username, $theme)
    {
        ...
        // Include the file
        include_once dirname($_SERVER['DOCUMENT_ROOT']) . '/resources/views/' . 'footer.php';
    ...
    {
        return require_once dirname($_SERVER['DOCUMENT_ROOT']) . '/resources/views/' . $route;
    }
}
